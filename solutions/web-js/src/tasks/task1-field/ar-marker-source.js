function createMarkerEntity(definition) {
  ...
  label.setAttribute('value', definition.id);
...
export function createArFieldMarkerSource({ scene, target = scene, registry, definitions }) {
  const markerEntities = definitions.map((definition) => {
    const markerEntity = createMarkerEntity(definition);
    target.appendChild(markerEntity);
